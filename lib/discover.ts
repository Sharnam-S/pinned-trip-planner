...
- If interests are given, devote roughly half the queries to them (e.g. interest "skiing" near Tbilisi -> "Gudauri skiing vlog") — but never ALL queries; general coverage must remain. Translate an interest into the spread of *places* across the destination that satisfy it, not a single venue or happening — "party" in Belgium means the nightlife of Brussels and Antwerp, the best bars, clubs, and speakeasies, NOT one music festival.
- The app plots every spot on a map, so a trip only works when its places are spread across the destination. Never build queries around a single one-off, ticketed, or single-location event — a festival like Tomorrowland, a concert, a marathon — because every video about it points at the same coordinates. The only exception is when the traveler explicitly named that event, or gave travel dates that fall inside it. Otherwise steer the interest toward the standing venues and neighborhoods someone can visit any week of the year.
